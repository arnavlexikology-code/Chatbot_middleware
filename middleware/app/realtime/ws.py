...
# ============================================================================
# RealtimeManager Wrapper
# ============================================================================

class RealtimeManager:
    """
    Wrapper class for publishing events to WebSocket connections.
    
    Provides a simple publish() method that broadcasts events to conversation rooms.
    Manages typing state cache for assistant.typing events.
    """
    
    def __init__(self):
        self.manager = manager
        # Internal TTL store for typing states
        # Format: {conversationId: {"timestamp": datetime, "event": dict}}
        self._typing_store: Dict[str, Dict[str, Any]] = {}
        self._typing_lock = asyncio.Lock()
        self._typing_ttl_seconds = 5
    
    def _get_typing_cache(self):
        """
        Get typing cache from events module if available, otherwise use internal store.
        
        Returns:
            Typing cache instance or None if not available
        """
        try:
            from .events import typing_cache
            return typing_cache
    ...
    async def publish(self, event: Dict[str, Any]) -> int:
        ...
        If event.type starts with "assistant.typing", updates TTL cache.
        
        Args:
            event: Event dictionary with at least 'type' and 'conversationId' fields
        ...
        if not conversation_id:
            logger.warning("Event missing conversationId, cannot publish")
            return 0
        
        event_type = event.get("type", "")
        
        # Handle typing state cache for assistant.typing events
        if event_type.startswith("assistant.typing"):
            await self._update_typing_cache(conversation_id, event_type, event)
        
        # Broadcast to WebSocket subscribers
        return await self.manager.broadcast_to_conversation(conversation_id, event)
    
    async def _update_typing_cache(
        self,
        conversation_id: str,
        event_type: str,
        event: Dict[str, Any]
        ...
        """
        Update typing state cache for assistant.typing events.
        
        Tries to use events module cache first, falls back to internal store.
        
        Args:
            conversation_id: Conversation identifier
            event_type: Event type (e.g., "assistant.typing.start")
            event: Full event dictionary
        """
        typing_cache = self._get_typing_cache()
        
        if typing_cache:
            # Use events module cache
            try:
                if event_type in ("assistant.typing.start", "assistant.typing.progress"):
                    typing_state = {
                        "typingState": {
                            ...
                            "partialText": event.get("partialText")
                        }
                    }
                ...
                logger.warning(
                    f"Failed to update typing cache via events module: {str(e)}",
                    exc_info=True
                )
                # Fall back to internal store
                await self._update_internal_typing_store(conversation_id, event_type, event)
        else:
            # Use internal store
            await self._update_internal_typing_store(conversation_id, event_type, event)
    
    async def _update_internal_typing_store(
        self,
        conversation_id: str,
        event_type: str,
        event: Dict[str, Any]
        ...
        """
        Update internal typing state store.
        ...
        async with self._typing_lock:
            if event_type in ("assistant.typing.start", "assistant.typing.progress"):
                # Store with timestamp and event payload
                self._typing_store[conversation_id] = {
                    "timestamp": datetime.utcnow(),
                    "event": event
                }
                logger.debug(
                    f"Stored typing state for conversation {conversation_id} in internal store"
                )
            elif event_type == "assistant.typing.end":
                # Remove from store
                if conversation_id in self._typing_store:
                    del self._typing_store[conversation_id]
                    logger.debug(
                        f"Removed typing state for conversation {conversation_id} from internal store"
                    )
    
    async def get_typing_state(self, conversation_id: str) -> Optional[Dict[str, Any]]:
        """
        Get the last typing state for a conversation.
        
        Tries to use events module cache first, falls back to internal store.
        Automatically removes expired entries from internal store.
        ...
        if typing_cache:
            # Use events module cache
            try:
                state = await typing_cache.get_typing(conversation_id)
                if state:
                    return state
            except Exception as e:
                logger.warning(
                    f"Failed to get typing state from events module: {str(e)}",
                    exc_info=True
                )
                # Fall back to internal store
                return await self._get_internal_typing_state(conversation_id)
        else:
            # Use internal store
            return await self._get_internal_typing_state(conversation_id)
    
    async def _get_internal_typing_state(
        self,
        conversation_id: str
    ) -> Optional[Dict[str, Any]]:
        """
        Get typing state from internal store.
        ...
        Returns:
            Event payload if found and not expired, None otherwise
        """
        async with self._typing_lock:
            ...
            for key, value in self._typing_store.items():
                stored_time = value.get("timestamp")
                if stored_time:
                    if isinstance(stored_time, str):
                        try:
                            stored_time = datetime.fromisoformat(stored_time.replace("Z", ""))
                        except (ValueError, AttributeError):
                            expired_keys.append(key)
                            continue
                    
                    age = (now - stored_time).total_seconds()
                    if age >= self._typing_ttl_seconds:
                        expired_keys.append(key)
            
            for key in expired_keys:
                del self._typing_store[key]
                logger.debug(f"Removed expired typing state for conversation {key}")
            
            # Return requested state if it exists
            if conversation_id in self._typing_store:
                stored = self._typing_store[conversation_id]
...
__all__ = ["realtime_router", "manager", "realtime_manager"]